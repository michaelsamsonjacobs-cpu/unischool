        ...
        <div className="flex flex-col pt-20">
            ...
            <section className="bg-slate-900 py-20 border-b border-white/5">
                <div className="max-w-7xl mx-auto px-4 text-center">
                    <span className="text-[#C9B47C] font-bold tracking-widest text-xs uppercase mb-4 block">For The Protagonist</span>
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        {[
                            ...
                            },
                            {
                                ...
                                desc: "No busy work. Earn XP by building drones, writing persuasive essays, or launching a business."
                            },
                            {
                                icon: Shield,
                            ...
                            <div key={i} className="bg-slate-800/50 border border-slate-700 p-8 rounded-2xl hover:border-[#8B2332]/50 transition-colors">
            ...
            <section className="py-24 bg-slate-900">
                <div className="max-w-7xl mx-auto px-4">
                    <div className="flex items-center justify-between mb-12">
                        <div>
                            <h2 className="font-serif text-3xl font-bold text-white mb-2">Guaranteed Admissions</h2>
                            <p className="text-slate-400">Just a few of our 60+ university partners.</p>
                        </div>
                        <button
                            onClick={() => onNavigate('partners')}
                            className="flex items-center gap-2 text-[#C9B47C] font-semibold hover:text-[#e5d5a8]"
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {topPartners.map((uni, i) => (
                            <div key={i} className="bg-[#0f172a] border border-slate-700 p-6 rounded-xl flex items-start gap-4">
                                <div className="h-12 w-12 bg-white rounded-lg flex items-center justify-center font-serif font-bold text-slate-900 text-xl flex-shrink-0">
                                    {uni.name.charAt(0)}
                                </div>
                                <div>
                                    <h4 className="font-bold text-white text-lg">{uni.name}</h4>
                                    <div className="flex flex-wrap gap-2 mt-2">
                                        <span className="text-xs bg-[#C9B47C]/10 text-[#C9B47C] px-2 py-1 rounded">GPA {uni.minGPA}+</span>
                                        {uni.online && <span className="text-xs bg-slate-700 text-slate-300 px-2 py-1 rounded">Online Available</span>}
                                    </div>
